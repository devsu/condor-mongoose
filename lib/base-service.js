...
const ACTION = {'PUSH': 0, 'ADD_TO_SET': 1, 'REMOVE': 2, 'REPLACE': 3, 'UPDATE': 4};
...
function getSubDocumentMethods(Model) {
  return {
    'push': function(call) {
      return runSubDocumentRequest(Model, call.request, ACTION.PUSH);
    },
    'addToSet': function(call) {
      return runSubDocumentRequest(Model, call.request, ACTION.ADD_TO_SET);
    },
    'remove': function(call) {
      return runSubDocumentRequest(Model, call.request, ACTION.REMOVE);
    },
    'update': function(call) {
      return runSubDocumentRequest(Model, call.request, ACTION.UPDATE);
    },
    'replace': function(call) {
      return runSubDocumentRequest(Model, call.request, ACTION.REPLACE);
    },
  };
}
...
function executeSubDocumentAction(request, model, action) {
  ...
  request[fieldName].forEach((child) => {
    if (action === ACTION.PUSH || action === ACTION.REPLACE) {
      model[fieldName].push(child);
    }
    if (action === ACTION.ADD_TO_SET) {
...
function getRelatedModelMethods(Model) {
  return {
    'push': function(call) {
      return runRelatedModelRequest(Model, call.request, ACTION.PUSH);
    },
    'addToSet': function(call) {
      return runRelatedModelRequest(Model, call.request, ACTION.ADD_TO_SET);
    },
    'remove': function(call) {
      return runRelatedModelRequest(Model, call.request, ACTION.REMOVE);
    },
    'replace': function(call) {
      return runRelatedModelRequest(Model, call.request, ACTION.REPLACE);
    },
  };
}
