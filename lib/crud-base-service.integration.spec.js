...
describe('Crud Base Service', () => {
  ...
  beforeEach((done) => {
    ...
    const defaultModel1 = {'name': 'Juan Pablo', 'age': 33, 'married': false, 'children': []};
  ...
  describe('insert()', () => {
    ...
    beforeEach(() => {
      newModel = {'name': 'Gaby', 'age': 23, 'married': false, 'children': []};
  ...
  describe('get()', () => {
    ...
    describe('getRequest with populate', () => {
      ...
      it('should get the queried document from the database with related model', (done) => {
        clientGet({'id': modelToQuery.id, 'populate': ['relatedModels', 'invalidRelatedField']})
          .then((response) => {
            expect(response).toEqual(jasmine.objectContaining(modelToQuery));
            done();
          });
