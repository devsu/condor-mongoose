...
const MongooseUtils = require('../spec/util/mongoose-utils');
...
describe('Crud Base Service', () => {
  ...
  beforeAll((done) => {
    MongooseUtils.initializeMongoose()
      .then(() => {
  ...
  afterAll((done) => {
    condor.stop()
      .then(() => {
        return MongooseUtils.disconnectMongoose();
      })
      .then(done);
  });
    ...
    describe('delete()', () => {
      ...
      describe('registry not found', () => {
        it('should return model not found error', (done) => {
          clientDelete(modelNotStored.id).catch((error) => {
  ...
  function initializeCondorClient() {
    ...
    client = new coreProto.smartmate.SamplesService('127.0.0.1:50051', creds);
    alternativeClient = new coreProto.smartmate.AlternativeService('127.0.0.1:50051', creds);
  }
