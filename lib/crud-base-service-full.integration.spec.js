...
describe('Crud Base Service Full', () => {
  let condor, client, alternativeClient, model1, relatedModel1, relatedModel2, child,
    expectedChildren;
  ...
  afterEach((done) => {
    if (mongoose.connection.readyState === 1) {
      return Promise.all([Model.remove({}), AlternativeModel.remove({})]).then(done);
    }
    TestUtils.initializeMongoose()
      .then(() => {
        return Promise.all([Model.remove({}), AlternativeModel.remove({})]);
      })
      .then(done);
  });
  ...
  function initializeCondorClient() {
    ...
    alternativeClient = new coreProto.smartmate
      .AlternativeService('127.0.0.1:3000', grpc.credentials.createInsecure());
  }
});
